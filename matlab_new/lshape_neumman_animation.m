...
errvals_inf(step)=max(max(abs([u_up(:); u_lo(:)]-[u_up_true(:); u_lo_true(:)])));
...
Du_dy=(ZZ(3:Nplot,:)-ZZ(1:Nplot-2,:))/((2*dy)/(Nplot-1));
Du_dx=(ZZ(:,3:Nplot)-ZZ(:,1:Nplot-2))/((2*dx)/(Nplot-1));
...
errvals_H1(step)=sqrt(sum(integrals));
end
...
figure(4)
nn = 1:Nsteps;
plot(nn,[errvals_inf; errvals_H1]);
semilogy(nn,[errvals_inf;errvals_H1]);
legend('inf norm','H1 norm')
toc
